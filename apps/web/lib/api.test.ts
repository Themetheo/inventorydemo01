...
describe("api error handling", () => {
  it("classifies a non-JSON server failure as an unavailable backend and logs safe diagnostics", async () => {
    ...
    expect(consoleWarn).toHaveBeenCalledWith("[api] Request failed", {
      method: "GET",
    ...
    expect(JSON.stringify(consoleWarn.mock.calls)).not.toContain("internal detail");
    expect(JSON.stringify(consoleWarn.mock.calls)).not.toContain("private-value");
  });
  ...
  it.each([
    ...
    expect(consoleWarn).toHaveBeenCalledWith("[api] Request failed", expect.objectContaining({
      classification: "BACKEND_UNAVAILABLE",
      status: 502,
    }));
    expect(JSON.stringify(consoleWarn.mock.calls)).not.toContain("internal upstream detail");
    expect(JSON.stringify(consoleWarn.mock.calls)).not.toContain("untrusted gateway detail");
  });
