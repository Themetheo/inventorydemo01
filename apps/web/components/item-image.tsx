...
export function ItemImage({ src, itemName, alt, className = "" }: ItemImageProps) {
  ...
  return <div className={`grid aspect-square min-w-0 place-items-center overflow-hidden bg-amber-50 ${className}`}>
    ...
    /> : <div className="grid size-20 min-h-20 min-w-20 place-items-center border-2 border-black bg-amber-100 p-4 shadow-[5px_5px_0_#18130f] box-border sm:size-24 sm:min-h-24 sm:min-w-24" aria-label={`ไม่มีรูป ${itemName}`} data-testid="item-placeholder">
